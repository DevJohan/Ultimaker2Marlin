...
typedef void(*i2cCallback)(i2cCommand*);
 ...
 */
static inline void i2cDriverCommandSetup(i2cCommand& command, uint8_t address_rw, uint8_t priority, uint8_t* buffer, uint8_t buffer_size, i2cCallback callback = 0)
{
    command.priority = priority;
    command.finished = true;
    command.finnish_callback = callback;
...
/**
    ...
    Set the priority (higher priority commands take preference over lower)
 */
static inline void i2cDriverCommandSetup(i2cCommand& command, uint8_t priority, i2cCallback callback = 0 )
{
    command.priority = priority;
    command.finished = true;
    command.finnish_callback = callback;
		...
		uint8_t buffer_size)
{
	...
	if(command.command_sequence_size >= max_sequences ){
		return; //TODO: should perhaps signal error
	}
